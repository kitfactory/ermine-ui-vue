...
今、ブラウザで閲覧しているWebサイトはHTMLと呼ばれる構造化文書で記述されています。HTMLはW3C、現在はWhatWGにより仕様が検討されています。

## 演習1
一番簡単なHTMLを作ってブラウザで確認してみましょう。
...
HTMLは要素からできています。簡単な要素を覚えておきましょう。

|要素|意味|
|:--|:--|
|html|HTML文書|
|head|文書のヘッダ情報です。タイトル等を記載します。|
|body|具体的なHTMLの中身|
|h1...hn|見出し文字です|
|div|単体では特に意味を持たない要素ですが、コンポーネントでよく利用されます。|
|form|入力の送信の塊を作ります|
|input|入力の項目です。type指定で様々な入力項目を実現します。|
|textara|文字の入力/複数行の場合に使用します。|
|button|ボタンです。|

# CSS

CSS(Cascade Style Sheet)はデザインや最近ではアニメーションを制御します。要素にはクラスやIDを指定することができるので、セレクタで指定された要素に、デザインを適用します。
...
コンポーネントで画面要素を作っていくと、どうしても親や子となるコンポーネントと連携処理をする必要がでてきます。
...
this.$emit('イベント名',引数)でイベントを創出することができます。上位では、そのイベントを取得できるように、v-on:イベント名=""でメソッドを起動することができます。
...
なお、Mixinはグローバルにすることで、よりサービスっぽく使用することができます。
